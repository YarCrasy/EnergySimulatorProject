...
import type { DiagramConnection, DiagramNode, DiagramProjectData } from "@models/domain/simulator";
...
const resolveElementIdFromUiNode = (node: LooseRecord = {}) => (
	node?.elementId ??
	node?.meta?.id ??
	node?.meta?.elementId ??
...
const buildNodeSignature = (elementId: Identifier | null, positionX: unknown, positionY: unknown) => {
	if (!elementId) {
		return null;
	}
	return [
...
const attachBackendIdentifiers = (uiNodes: DiagramNode[], apiNodes: LooseRecord[] = []) => {
	if (!apiNodes.length) {
		return uiNodes;
	}
...
const serializeDiagram = (nodes: DiagramNode[], connections: DiagramConnection[]) => JSON.stringify({
...
const normalizeProject = (project: DiagramProjectData) => {
...
const buildProjectPayload = (project: DiagramProjectData, nodes: DiagramNode[], connections: DiagramConnection[]) => {
...
const buildElementDictionary = (elements: EnergyElement[] = []) => {
	const catalog = new Map<Identifier, EnergyElement>();
...
const buildElementMetadataCache = (nodes: DiagramNode[] = []) => {
	const cache = new Map<Identifier, LooseRecord>();
	nodes.forEach((node) => {
		const elementId = resolveElementIdFromUiNode(node);
		if (!elementId) {
			return;
...
const hydrateNodesWithMetadata = (nodes: DiagramNode[] = [], cache?: Map<Identifier, LooseRecord> | null) => {
...
const hydrateNodesFromCatalog = (nodes: DiagramNode[] = [], catalog?: Map<Identifier, EnergyElement> | null) => {
