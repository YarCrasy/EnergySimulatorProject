...
function DiagramWorkspace({ projectId }: DiagramWorkspaceProps) {
    ...
    const [projectData, setProjectData] = useState<DiagramProjectData | null>(null);
    const [elementCatalog, setElementCatalog] = useState<Map<Identifier, EnergyElement> | null>(null);
    ...
    const addNode = useCallback((partialNode: Partial<DiagramNode> = {}, position?: DiagramNode["position"]) => {
    ...
    const moveNode = useCallback((nodeId: string, position: DiagramNode["position"]) => {
    ...
    const dropElement = useCallback((payload: LooseRecord | null, position?: DiagramNode["position"]) => {
        if (!payload) {
            return;
        }
    ...
    const requestConnection = useCallback((nodeId: string) => {
