-- *******************************************
--  Datos de demostración para energysimulatordb
--  Se ejecuta automáticamente tras arrancar el backend
--  (Spring Boot/JPA ya ha creado las tablas)
-- *******************************************

-- 4) Usuarios de aplicación (admin + prueba) si existe tabla users
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
...
-- 5) Elemento consumidor de prueba
DO $$
BEGIN
...
-- 6) Elemento generador de prueba
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'elements')
...
-- 7) Elemento de almacenamiento de prueba (batería)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'elements')
