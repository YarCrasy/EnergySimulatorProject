...
describe("tests/Interfaces/ProjectCard/actions", () => {
  it("boton de menu accesible abre contexto con coordenadas del boton", () => {
    // Arrange
    renderCard();
    ...
    // Assert
    expect(screen.getByRole("menu")).toHaveStyle({ top: "654px", left: "321px" });
    expect(screen.getByText("Eliminar")).toBeInTheDocument();
  });

  it("sin onOpen, el boton Abrir simulador navega por id", () => {
    // Arrange
    renderCard();
  ...
  it("con onOpen, el boton Abrir simulador delega en callback", () => {
    // Arrange
    const onOpen = vi.fn();
    renderCard({ onOpen });
  ...
  it("con onDelete, opcion Eliminar delega en callback", () => {
    // Arrange
  ...
  it("sin onDelete, confirma eliminacion y registra log", () => {
    // Arrange
    ...
    // Assert
    expect(confirmSpy).toHaveBeenCalledWith(expect.stringContaining("Eliminar el proyecto"));
    expect(logSpy).toHaveBeenCalledWith("Eliminar proyecto:", 42);
  });

  it("si el menu esta abierto, click sobre la card solo cierra menu y no ejecuta onOpen", () => {
    // Arrange
    const onOpen = vi.fn();
  ...
  it("click sobre la card llama onOpen cuando el menu no esta abierto", () => {
    // Arrange
    const onOpen = vi.fn();
    const { container } = renderCard({ onOpen });
    const link = container.querySelector(".project-card");
  ...
  it("touchend limpia timeout pendiente sin abrir menu", () => {
    // Arrange
    vi.useFakeTimers();
    const { container } = renderCard();
    const shell = container.querySelector(".project-card-shell");
  ...
  it("ignora touchstart con mas de un touch y no abre menu", () => {
    // Arrange
    vi.useFakeTimers();
    const { container } = renderCard();
    const shell = container.querySelector(".project-card-shell");
  ...
  it("long press abre menu y touchend hace preventDefault", () => {
    // Arrange
    vi.useFakeTimers();
    const { container } = renderCard();
    const shell = container.querySelector(".project-card-shell");
  ...
  it("touchmove y touchcancel cancelan long press antes de abrir menu", () => {
    // Arrange
    vi.useFakeTimers();
    const { container } = renderCard();
    const shell = container.querySelector(".project-card-shell");
